...
%   NOT ANYMORE. This script will prompt the original ui control window.
%   Clicking the RX button will run the hackrf and plot one dataset based
%   on the parameters specified in the rxbut case switch in the main
%   callback function.
    ...
    function callback(g,~)
        ...
        switch g
            ...
            case rxbut
                if get(g,'Value') % this portion of the callback has been modified to populate settings as the RX start button is pressed
                    h.rxStart;
                    changefreq(444000000) % change tune frequency here
                    h.SampleRate = 2e7;
                    h.lnaGain = 8; % [0:8:40]
    ...
    function rxcallback(~,z)
        ...
        % process one data set and quit
        xl=h.Frequency+r/q/2*[-1 1]*h.SampleRate; % calculate bandwidth based on tune freq and sample rate
        xd=linspace(xl(1),xl(2),n); % create a frequency axis vector with the number of FFT points
        
        figure
        plot(xd,Z) % plot spectrum
        pause(1) %  stream should have completed already but this is necessary or MATLAB will probably crash
        h.rxStop; % stop RX stream!!
%         save('data.mat','Z')
